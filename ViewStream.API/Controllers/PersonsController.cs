...
[ApiController]
[Route("api/v1/admin/persons")]
[EnableRateLimiting("AdminRateLimit")]
[Authorize(Roles = "SuperAdmin,ContentManager,Moderator")]
[Produces("application/json")]
public class AdminPersonsController : ControllerBase
{
    private readonly IMediator _mediator;

    public AdminPersonsController(IMediator mediator) => _mediator = mediator;

    /// <summary>
    /// Retrieves a paginated list of persons for the admin dashboard.
    /// </summary>
    ...
    public async Task<ActionResult<PagedResult<AdminPersonListItemDto>>> GetAdminPaged(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = new ViewStream.Application.Queries.Person.GetAdminPersonsPaged.GetAdminPersonsPagedQuery(pageNumber, pageSize, searchTerm, sortBy, sortDescending, includeDeleted);
