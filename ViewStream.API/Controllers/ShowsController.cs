...
public class AdminShowsController : ControllerBase
{
    ...
    public AdminShowsController(IMediator mediator) => _mediator = mediator;

    /// <summary>
    /// Retrieves a paginated list of shows for the admin dashboard.
    /// </summary>
    ...
    public async Task<ActionResult<PagedResult<AdminShowListItemDto>>> GetAdminPaged(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = new GetAdminShowsPagedQuery(pageNumber, pageSize, searchTerm, sortBy, sortDescending, includeDeleted);
