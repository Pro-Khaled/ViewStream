...
namespace ViewStream.API.Controllers
{
    ...
    public class ShowsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadPoster(long id, IFormFile posterFile, CancellationToken cancellationToken)
        {
            var userId = long.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        ...
        [Authorize(Roles = "ContentManager,SuperAdmin")]
        [ProducesResponseType(typeof(string), StatusCodes.Status200OK)]
        public async Task<IActionResult> UploadBackdrop(long id, IFormFile backdropFile, CancellationToken cancellationToken)
        {
            var userId = long.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        ...
        [Authorize(Roles = "ContentManager,SuperAdmin")]
        [ProducesResponseType(typeof(string), StatusCodes.Status200OK)]
        public async Task<IActionResult> UploadTrailer(long id, IFormFile trailerFile, CancellationToken cancellationToken)
        {
            var userId = long.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
