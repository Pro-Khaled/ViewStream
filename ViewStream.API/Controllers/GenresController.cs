...
public class AdminGenresController : ControllerBase
{
    ...
    public AdminGenresController(IMediator mediator) => _mediator = mediator;

    /// <summary>
    /// Retrieves a paginated list of genres for the admin dashboard.
    /// </summary>
    ...
    public async Task<ActionResult<PagedResult<AdminGenreListItemDto>>> GetAdminPaged(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = new GetAdminGenresPagedQuery(pageNumber, pageSize, searchTerm, sortBy, sortDescending, includeDeleted);
