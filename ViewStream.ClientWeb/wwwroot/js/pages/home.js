pages.home = (() => {
    ...
    async function load() {
        ...
        try {
            const [trending, top, news, action, drama, scifi, cw] = await Promise.all([
                api.get('/shows', { page: 1, pageSize: 12, orderBy: 'imdb_rating', isDescending: true }),
                api.get('/shows', { page: 1, pageSize: 12, orderBy: 'rotten_tomatoes_score', isDescending: true }),
                api.get('/shows', { page: 1, pageSize: 8, year: new Date().getFullYear() }),
                api.get('/shows', { page: 1, pageSize: 12, genreId: 1 }),
                api.get('/shows', { page: 1, pageSize: 12, genreId: 2 }),
                api.get('/shows', { page: 1, pageSize: 12, genreId: 5 }),
                store.isAuthenticated() ? api.get('/profiles/me/history/continue?limit=10') : Promise.resolve([])
            ]);
            state.data = { trending, top, news, action, drama, scifi, cw };
        } catch (err) { toast.error('Failed to load home'); }
    ...
    function render() {
        ...
        if (state.loading) {
            ...
            const { trending, top, news, action, drama, scifi, cw } = state.data;
            ...
            h += Comp.showRow('Action & Adventure', action.items);
            h += Comp.showRow('Drama', drama.items);
            h += Comp.showRow('Sci-Fi & Fantasy', scifi.items);
        }
        c.innerHTML = h;
    }
