pages.library = (() => {
    ...
    async function load() {
        ...
        } catch (err) { toast.error('Failed to load library'); state.data = null; }
    ...
    function render() {
        ...
        if (!c) return;
        let h = Comp.pageHeader('My Library', `${state.data?.totalCount || 0} items`);
        h += `<div class="flex gap-2 mb-6 overflow-x-auto pb-1" id="status-filters">
            ${['all', 'watching', 'completed', 'plan_to_watch', 'on_hold', 'dropped'].map(s => `<button class="px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${s === state.status ? 'bg-vs-accent text-vs-bg' : 'bg-vs-card text-vs-dim hover:bg-vs-elevated border border-vs-border'}" data-status="${s}">${s.replace('_', ' ')}</button>`).join('')}
        </div>`;
        if (state.loading) { h += Comp.pageLoader(); }
        else if (!state.data?.items?.length) { h += Comp.emptyState('fa-bookmark', 'Your library is empty').replace('Your library is empty', 'Your library is empty<p class="mt-2"><a href="#/shows" class="text-vs-accent">Browse Shows</a></p>'); }
        else {
            h += `<div class="grid gap-4" id="library-grid">
                ...
                return `<div class="flex gap-4 p-4 rounded-xl bg-vs-surface border border-vs-border hover:border-vs-dim transition-colors library-item" data-item-id="${item.id}">
                        <a href="#/shows/${item.showId}" class="flex-shrink-0">
                            <img src="${toast.esc(item.posterUrl || item.showPosterUrl)}" alt="" class="w-24 h-36 rounded-lg object-cover">
                        </a>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <div>
                                    <a href="#/shows/${item.showId}" class="font-display font-semibold text-vs-text hover:text-vs-accent">${toast.esc(item.title || item.showTitle)}</a>
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <button class="p-1.5 text-vs-muted hover:text-vs-error library-delete-btn"><i class="fas fa-trash-alt text-xs"></i></button>
                                </div>
                            </div>
    ...
    return {
        ...
        init() {
            ...
            document.getElementById('status-filters')?.addEventListener('click', e => {
                ...
                if (btn) filterByStatus(btn.dataset.status);
            });
            document.getElementById('library-grid')?.addEventListener('change', e => {
                if (e.target.classList.contains('library-status-select')) {
                    ...
                    api.put(`/profiles/me/library/${id}`, { status: e.target.value }).then(() => toast.success('Status updated')).catch(err => toast.error(err.message));
                }
            });
            document.getElementById('library-grid')?.addEventListener('click', e => {
                if (e.target.closest('.library-delete-btn')) {
                    const id = parseInt(e.target.closest('.library-item').dataset.itemId);
                    api.del(`/profiles/me/library/${id}`).then(() => { toast.success('Removed'); load(); }).catch(err => toast.error(err.message));
                }
            });
        },
